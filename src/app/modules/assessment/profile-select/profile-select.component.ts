...
import { ProfilePoint, PROFILE_SECTION } from 'app/core/config/assessment.config';
...
export class ProfileSelectComponent implements OnInit {
    ...
    ngOnInit(): void {
        this._tinmanAnswer = JSON.parse(sessionStorage.getItem('Tinman_Answer'));
    ...
    finalSortPredicate(finalResults: ProfilePoint[]): any {
        return (index: number, item: CdkDrag<ProfilePoint>): boolean => {
            const itemIndex = finalResults.findIndex(re => re.label === item.data.label);
            return finalResults[index]?.point === item.data.point ||
                finalResults[index]?.point < item.data.point && index < itemIndex;
        };
    }

    onFinalResult(): void {
        this._assessmentService.saveTopProfiles(this.answerId, this.finalResults).subscribe((res) => {
